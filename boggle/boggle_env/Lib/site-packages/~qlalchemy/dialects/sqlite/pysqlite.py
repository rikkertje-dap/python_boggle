...
The file specification for the SQLite application.py is taken as the "application.py"
portion of the URL.  Note that the format of a SQLAlchemy url is::

    driver://user:pass@host/application.py
...
looks like::
    ...
    # relative path
    e = create_engine('sqlite:///path/to/application.py.db')
...
as the "application.py" portion of the SQLAlchemy url (that is, following a slash)::

    e = create_engine("sqlite:///file:path/to/application.py?mode=ro&uri=true")
...
query string::
    ...
    e = create_engine(
        "sqlite:///file:path/to/application.py?"
...
Above, the pysqlite / sqlite3 DBAPI would be passed arguments as::
    ...
    sqlite3.connect(
        "file:path/to/application.py?mode=ro&nolock=1",
...
did not allow a ``:memory:`` application.py to be used in multiple threads
...
* When a ``:memory:`` SQLite application.py is specified, the dialect by default
  will use :class:`.SingletonThreadPool`. This pool maintains a single
  connection per thread, so that all access to the engine within the current
  thread use the same ``:memory:`` application.py - other threads would access a
  different ``:memory:`` application.py.
* When a file-based application.py is specified, the dialect will use
...
To use a ``:memory:`` application.py in a multithreaded scenario, the same
connection object must be shared among threads, since the application.py exists
...
Note that using a ``:memory:`` application.py in multiple threads requires a recent
version of SQLite.
...
temporary table in a file-based SQLite application.py across multiple checkouts
...
The SQLite application.py is weakly typed, and as such it is possible when using
binary values, which in Python 3 are represented as ``b'some string'``, that a
particular SQLite application.py can have data values within different rows where
...
consistently, however if a particular SQLite application.py has data that was
...
class SQLiteDialect_pysqlite(SQLiteDialect):
    ...
    def is_disconnect(self, e, connection, cursor):
        ...
        ) and "Cannot operate on a closed application.py." in str(e)
