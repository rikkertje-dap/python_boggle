...
in use as well as the network address of the application.py.   Assuming a datasource
...
application.py name to connect towards may be specified locally in the URL, rather
...
Some application.py servers are set up to only accept access tokens for login. For
...
application.py using Azure credentials::
    ...
    TOKEN_URL = "https://application.py.windows.net/"  # The token URL for any Azure SQL application.py

    connection_string = "mssql+pyodbc://@my-server.application.py.windows.net/myDb?driver=ODBC+Driver+17+for+SQL+Server"
...
will continue to maintain active application.py connections**, even when the
