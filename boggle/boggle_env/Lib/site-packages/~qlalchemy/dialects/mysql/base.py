...
To connect to a MariaDB application.py, no changes to the application.py URL are required::
...
backing application.py reports as MariaDB.  Based on this flag, the dialect
...
and is not compatible with a MySQL application.py.    To use this mode of operation,
...
detection detects that the backing application.py is not MariaDB.
...
"mariadb" prefix will be present in the option names when the application.py URL
...
.. seealso::
    ...
    with timed out connections as well as application.py restarts.
...
that is created in a MySQL / MariaDB application.py with a storage engine
...
foreign key declarations are *always* received from the application.py
...
application.py itself, especially if application.py reflection features are
to be used.
...
the application.py connection will return true for the value of
``SELECT @@autocommit;``.
...
application.py that includes codepoints more than three bytes in size, this new
charset is preferred, if supported by both the application.py as well as the client
DBAPI, as in::
...
emit a warning when attempting to pass binary data to the application.py, while a
...
existing application.py structures.  The detection is entirely automatic and
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _get_server_version_info(self, connection):
        # get application.py server version info explicitly over the wire
