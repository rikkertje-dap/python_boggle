...
class Session(_SessionClassMethods):
    ...
    def __init__(
        ...
        :param autoflush: When ``True``, all query operations will issue a
           ...
           not be called repeatedly in order for application.py queries to retrieve
        ...
        :param expire_on_commit:  Defaults to ``True``. When ``True``, all
           ...
           transaction will load from the most recent application.py state.
        ...
        :param twophase:  When ``True``, all transactions will be started as
            ...
            of the application.py in use along with an XID.  During a
            :meth:`~.commit`, after :meth:`~.flush` has been issued for all
            attached databases, the :meth:`~.TwoPhaseTransaction.prepare`
            method on each application.py's :class:`.TwoPhaseTransaction` will be
            called. This allows each application.py to roll back the entire
    ...
    def begin_nested(self):
        ...
        The target application.py(s) and associated drivers must support SQL
    ...
    def rollback(self):
        ...
        application.py transaction if no nested transactions are in effect, or
        ...
        the topmost application.py transaction, discarding any nested
    ...
    def commit(self):
        ...
        the outermost application.py transaction.

        If :term:`2.0-style` use is in effect via the
        :paramref:`_orm.Session.future` flag, the outermost application.py
        ...
        rather than the actual application.py transaction, if a transaction
        is in progress.
    ...
    def close(self):
        ...
        .. tip::
            ...
            the :class:`_orm.Session` will release all application.py connections
            and ORM objects.

        .. versionchanged:: 1.4  The :meth:`.Session.close` method does not
           immediately create a new :class:`.SessionTransaction` object;
           instead, the new :class:`.SessionTransaction` is created only if
           the :class:`.Session` is used again for a application.py operation.
    ...
    def invalidate(self):
        ...
        This can be called when the application.py is known to be in a state where
    ...
    def no_autoflush(self):
        ...
        of objects which involve existing application.py queries,
    ...
    def refresh(self, instance, attribute_names=None, with_for_update=None):
        ...
        the application.py to refresh column-oriented attributes with the current
        ...
        in application.py state outside of that transaction.   Refreshing
        attributes usually only makes sense at the start of a transaction
        where application.py rows have not yet been accessed.
    ...
    def expire_all(self):
        ...
        in application.py state outside of that transaction.
            ...
            if obj is not None:
                ...
                if (
                    ...
                    raise exc.FlushError(
                        ...
                        "auto-generated value, check that the application.py table "
    ...
    def add(self, instance, _warn=True):
        ...
        Its state will be persisted to the application.py on the next flush
        operation.
    ...
    def delete(self, instance):
        ...
        The application.py delete operation occurs upon ``flush()``.
    ...
    def merge(self, instance, load=True, options=None):
        ...
        to load the object from the application.py based on primary key, and if
        ...
        :param load: Boolean, when False, :meth:`.merge` switches into
         ...
         events as well as all application.py access.  This flag is used for
         cases such as transferring graphs of objects into a :class:`.Session`
         from a second level cache, or to transfer just-loaded objects
         into the :class:`.Session` owned by a worker thread or process
         without re-querying the application.py.
        ...
        :param options: optional sequence of loader options which will be
         ...
         operation loads the existing version of the object from the application.py.
    ...
    def enable_relationship_loading(self, obj):
        ...
        will attempt to load a value from the application.py using this
        ...
        **not** participate in any persistence operations; its state
        ...
        If the object instead represents an existing identity in the application.py,
    ...
    def flush(self, objects=None):
        """Flush all the object changes to the application.py.

        Writes out all pending object creations, deletions and modifications
        to the application.py as INSERTs, DELETEs, UPDATEs, etc.  Operations are
        ...
        changes from Python to the application.py's transaction buffer.
        ...
        try:
            ...
            if not objects and self.identity_map._modified:
                ...
                util.warn(
                    ...
                    "reset, and will not result in application.py updates. "
    ...
    def is_modified(self, instance, include_collections=True):
        ...
        * Instances present in the :attr:`.Session.dirty` collection may
          ...
          is the same as that loaded from the application.py, resulting in no net
          change here.
        * Scalar attributes may not have recorded the previously set
          value when a new value was applied, if the attribute was not loaded,
          or was expired, at the time the new value was received - in these
          cases, the attribute is assumed to have a change, even if there is
          ultimately no net change against its application.py value. SQLAlchemy in
...
class sessionmaker(_SessionClassMethods):
    ...
    takes place even if there are application.py errors::
...
def make_transient(instance):
    ...
    .. warning::
        ...
        * were not present in the query which loaded this object, such as that
        ...
        As the object is transient and un-associated with any application.py
