...
def identity_key(*args, **kwargs):
    ...
    * ``identity_key(instance=instance)``
      ...
      effect of performing a application.py check for the corresponding row
    ...
    * ``identity_key(class, row=row, identity_token=token)``
      ...
      application.py result row as a :class:`.Row` object.
...
def has_identity(object_):
    """Return True if the given object has a application.py
    identity.
