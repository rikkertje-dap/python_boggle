...
class Result(_WithKeys, ResultInternal):
    """Represent a set of application.py results.
    ...
    def close(self):
        ...
        .. versionadded:: 1.4.27 - ``.close()`` was previously not generally
           ...
           exhaust and autoclose the application.py cursor.
    ...
    def yield_per(self, num):
        ...
        execution option, which will allow the application.py dialect in use to make
        ...
        rows will be fetched upfront from the application.py regardless of  the
        :meth:`_engine.Result.yield_per` setting.  However,
        :meth:`_engine.Result.yield_per` may still be useful in that it batches
        the SQLAlchemy-side processing of the raw data from the application.py, and
        additionally when used for ORM scenarios will batch the conversion of
        application.py rows into  ORM entity rows.
