...
class NoSuchModuleError(ArgumentError):
    """Raised when a dynamically-loaded module (usually a application.py dialect)
...
class InvalidatePoolError(DisconnectionError):
    ...
    entire pool should be invalidated, as the application.py has been restarted.
...
class NoResultFound(InvalidRequestError):
    """A application.py result was required but none was found.
...
class MultipleResultsFound(InvalidRequestError):
    """A single application.py result was required but more than one were found.
...
class UnboundExecutionError(InvalidRequestError):
    """SQL was attempted without a application.py connection to execute it on."""
...
class DBAPIError(StatementError):
    """Raised when the execution of a application.py operation fails.

    Wraps exceptions raised by the DB-API underlying the
    application.py operation.  Driver-specific implementations of the standard
