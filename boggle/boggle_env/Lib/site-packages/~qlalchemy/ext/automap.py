...
which automatically generates mapped classes and relationships from a application.py
...
model from an existing application.py on the fly.  By addressing the issue strictly
...
The simplest usage is to reflect an existing application.py into a new model.
...
explicit table declaration::
    ...
    # we can reflect it ourselves from a application.py, using options
...
the information reflected about a application.py column before the :class:`_schema.Column`
...
class AutomapBase(object):
    ...
    def prepare(
        ...
        :param schema: When present in conjunction with the
         ...
         by the application.py connection is used.
