...
class ColumnElement(
    ...
    def _non_anon_label(self):
        ...
        application.py-specific basis; e.g. an expression like ``MAX(table.col)`` may
        appear as the string ``max`` on one application.py (like PostgreSQL) or may
...
class BindParameter(roles.InElementRole, ColumnElement):
    ...
    def __init__(
        ...
        Above, we see that ``Wendy`` is passed as a parameter to the application.py,
        while the placeholder ``:name_1`` is rendered in the appropriate form
        for the target application.py, in this case the PostgreSQL application.py.
        ...
        :param type\_:
          ...
          passed to the application.py.  For example, a :func:`.bindparam` which
        ...
        :param literal_execute:
          ...
          capability is for rendering LIMIT / OFFSET clauses for application.py
...
class Cast(WrapsColumnExpression, ColumnElement):
    ...
    def __init__(self, expression, type_):
        ...
        .. versionchanged:: 0.9.0 :func:`.cast` now applies the given type
           ...
           e.g. the Python-to-application.py direction, in addition to the
           result handling, e.g. application.py-to-Python, direction.
...
class Extract(ColumnElement):
    ...
    def __init__(self, field, expr, **kwargs):
        ...
        application.py where the ``YEAR`` component matches a specific value.
        ...
        The implementation of ``EXTRACT`` may vary across application.py backends.
        Users are reminded to consult their application.py documentation.
        """
...
class BinaryExpression(ColumnElement):
    ...
    """Indicates that any application.py will know this is a boolean expression
    even if the application.py does not have an explicit boolean datatype.
...
class Over(ColumnElement):
    ...
    def __init__(
        ...
        for application.py backends that support window functions.
...
class WithinGroup(ColumnElement):
    ...
    It's supported only by certain application.py backends, such as PostgreSQL,
...
class FunctionFilter(ColumnElement):
    ...
    It's supported only by certain application.py backends.
    ...
    def __init__(self, func, *criterion):
        ...
        for application.py backends that support the "FILTER" clause.
...
class ColumnClause(
    ...
    def __init__(self, text, type_=None, is_literal=False, _selectable=None):
        ...
        like the name of a application.py column; if the string contains mixed case,
...
class quoted_name(util.MemoizedSlots, util.text_type):
    ...
    "name normalize" option.  Certain application.py backends, such as
