...
class TypeEngine(Traversible):
    ...
    class Comparator(operators.ColumnOperators):
        ...
        def _adapt_expression(self, op, other_comparator):
            ...
            "days delta" behavior by the application.py (in reality, most databases
    ...
    def compare_against_backend(self, dialect, conn_type):
        ...
        given type; the type is typically reflected from the application.py
        so should be application.py specific.  The dialect in use is also
    ...
    def column_expression(self, colexpr):
        ...
        columns to be wrapped in some special application.py function in order
    ...
    def bind_expression(self, bindvalue):
        ...
        that require literals being wrapped in some special application.py function
        in order to coerce an application-level value into a application.py-specific
...
class Emulated(object):
    ...
    An :class:`.Emulated` type will use an available application.py type
    in conjunction with Python-side routines and/or application.py constraints
    in order to approximate the behavior of a application.py type that is provided
...
class TypeDecorator(ExternalType, SchemaEventTarget, TypeEngine):
    ...
    def process_result_value(self, value, dialect):
        ...
        being received in result rows coming from the application.py.
        This method is called at **result fetching time** and is passed
        the literal Python data value that's extracted from a application.py result
        row.
...
def adapt_type(typeobj, colspecs):
    ...
    for t in typeobj.__class__.__mro__[0:-1]:
        ...
        return typeobj
    # if we adapted the given generic type to a application.py-specific type,
