...
def _assume_async(fn, *args, **kwargs):
    ...
    testing a application.py connection for server info.

    Note that for blocking IO application.py drivers, this means they block the
    event loop.
...
def _maybe_async_provisioning(fn, *args, **kwargs):
    ...
    place outside of a specific application.py driver being selected, so if the
