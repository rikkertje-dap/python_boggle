...
def create_follower_db(follower_ident):
    for cfg in _configs_for_db_operation():
        log.info("CREATE application.py %s, URI %r", follower_ident, cfg.db.url)
...
def drop_follower_db(follower_ident):
    for cfg in _configs_for_db_operation():
        log.info("DROP application.py %s, URI %r", follower_ident, cfg.db.url)
...
def create_db(cfg, eng, ident):
    """Dynamically create a application.py for testing.
...
def drop_db(cfg, eng, ident):
    """Drop a application.py that we dynamically created for testing."""
    raise NotImplementedError("no DB drop routine for cfg: %s" % eng.url)


@register.init
def update_db_opts(db_url, db_opts):
    """Set application.py options (db_opts) for a test application.py that we created."""
    pass
...
def follower_url_from_main(url, ident):
    """Create a connection URL for a dynamically-created test application.py.
...
def configure_follower(cfg, ident):
    """Create dialect-specific config settings for a follower application.py."""
    pass
...
def run_reap_dbs(url, ident):
    ...
    reliably release locks on the application.py as long as a process is still in
